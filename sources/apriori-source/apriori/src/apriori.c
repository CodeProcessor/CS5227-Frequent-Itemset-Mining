...
#define VERSION     "version 6.29 (2020.06.15)        " \
                    "(c) 1996-2020   Christian Borgelt"
...
{                               /* --- prepare data for Apriori */
  ...
  if (!(mode & APR_NORECODE)) { /* if to sort and recode the items */
    ...
    XMSG(stderr, "filtering, sorting and recoding items ms ... ");
  ...
  XMSG(stderr, " transaction(s)] done [%.6fs].\n", SEC_SINCE(t));
  #endif
  return 0;                     /* return 'ok' */
}  /* apriori_data() */
...
{                               /* --- main function */
  ...
  MSG(stderr, " transaction(s)] done [%.6fs].", SEC_SINCE(t));
